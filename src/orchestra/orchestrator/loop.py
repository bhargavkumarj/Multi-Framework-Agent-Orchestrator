...
from orchestra.core.quiet import silence
...
from orchestra.orchestrator import (  # noqa: E402
    author_agent,
    catalog,
    live_board,
    prompts,
...
_APP = "agent_loop"
_MAX_TURNS = config.MAX_ORCHESTRATOR_CALLS
...
def _launch(goal_id: int, worker: dict, board_path: Path) -> subprocess.Popen:
    ...
    cwd = catalog.launch_cwd(worker)
    group = {} if sys.platform == "win32" else {"start_new_session": True}
    env = {**os.environ, "BOARD_PATH": str(board_path), "WORKER_MODEL": config.WORKER_MODEL}
    return subprocess.Popen(
        argv, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, cwd=cwd, env=env, **group
    )
...
def make_tools(team: Team) -> list:
    ...
    async def author_style() -> str:
        ...
        the builders start."""
        await author_agent.author_style(team.language, team.site_dir)
    ...
    async def build_hub() -> str:
        ...
        await author_agent.build_hub(team.language, games, team.site_dir)
...
def _ensure_site(team: Team) -> None:
    ...
    if not (team.site_dir / "common.css").exists():
        author_agent.write_template_style(team.site_dir)
    if not (team.site_dir / "index.html").exists():
        author_agent.write_template_hub(team.language, team.finished_games(), team.site_dir)
