...
one task, builds what the task asks, and exits, leaving the rest of the board alone.
    ...
    uv run pydantic_worker.py                       # standalone Day 2 demo
    uv run pydantic_worker.py <taskId> <boardPath>  # Day 5: work one task on a shared board
"""
...
# Standalone-demo scratch space, one per framework so the four demos do not
# overwrite each other. In team mode the work dir is the shared site instead.
WORKSPACE = Path(__file__).resolve().parents[3] / "workspace" / "pydantic"
